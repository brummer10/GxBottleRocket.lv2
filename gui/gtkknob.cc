...
#define GTK_BOTTLEROCKET_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_BOTTLEROCKET_KNOB, GtkBottleRocketKnobPrivate))

G_DEFINE_TYPE(GtkBottleRocketKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkBottleRocketKnobPrivate GtkBottleRocketKnobPrivate;

struct _GtkBottleRocketKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkBottleRocketKnobPrivate *priv = GTK_BOTTLEROCKET_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkBottleRocketKnob *knob = GTK_BOTTLEROCKET_KNOB(widget);
...
{
    g_assert(GTK_IS_BOTTLEROCKET_KNOB(widget));
    GtkBottleRocketKnob *knob = GTK_BOTTLEROCKET_KNOB(widget);
    GtkBottleRocketKnobPrivate *priv = GTK_BOTTLEROCKET_KNOB_GET_PRIVATE(knob);
...
{
    GtkBottleRocketKnob *knob = GTK_BOTTLEROCKET_KNOB(object);
...
/****************************************************************
 ** init the GtkBottleRocketKnobClass
 */

static void gtk_knob_class_init (GtkBottleRocketKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkBottleRocketKnobPrivate));
}
...
static void gtk_knob_init (GtkBottleRocketKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_BOTTLEROCKET_KNOB, NULL ));
